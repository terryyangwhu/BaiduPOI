...
class BaiDuPOI_City(object):
    ...
    def urls(self):
        ...
        for pages in range(0,50):
            url = 'http://api.map.baidu.com/place/v2/search?query='+ self.itemy + '&region='+ self.city + '&city_limit=true&scope=2&page_size=20&page_num=' + str(pages) + '&output=json&ak=' + self.api_key
            urls.append(url)
        return urls
